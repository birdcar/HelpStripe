...
**Active Feature:** feat-004 complete — Phase 4 (Self-Service Portal) done. Remaining: **feat-006 Automation** only.
...
1. **feat-006 Automation Rules** (depends on feat-003 ✅) — the ONLY remaining feature. The Mail Rules seam is shipped: `ProcessInboundEmail::applyMailRules()` is a no-op pass-through Phase 6 fills (without reopening the inbound pipeline). Triggers attach to the existing domain events (RequestCreated/NoteAdded/RequestStatusChanged/RequestAssigned) exactly like SendPublicReplyEmail. Scheduled Automation Rules layer = `automation:run` + scheduler; reuse `Request::scopeSlaBreached`/`scopeSlaOverdue` (Phase 8) for any SLA condition — never re-derive. Spec: docs/ideation/helpstripe/spec-phase-6.md.
